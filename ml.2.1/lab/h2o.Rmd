...
You might use H2O's automl functionality that does the model search and hyperparameter tuning for you.
You can use a lot of settings to limit the runtime or the number of tried models.
You might also define a `leaderboard_frame` if you would like to calculate the performance metrics on a different data set instead of on the hold-out samples.
...
automl_fit@leaderboard
```

```{r}
# Compare the performance of the best model to our previous models
c(
  validation_performances,
  automl = h2o.rmse(h2o.performance(automl_fit@leader, data_valid))
)
```

As you can see, there is some gain from using the auto-ML feature, but it does not change the big picture:
each model gives us an RMSE of about 0.26. This corresponds to a mean absolute error of `r round(h2o.mae(h2o.performance(automl_fit@leader, data_valid)), 2)`, meaning that we are off about 20% on average. To improve this performance, you probably need additional data instead of more sophisticated models.
